...
export function derive(t, asOf = todayISO()) {
  ...
  for (const s of FX_STAGES) {
    fx[s] = {
      ...
      source: pegged ? 'peg' : t.fx_source ?? null,
    };
  }
...
export function summaryReport(trades, asOf = todayISO()) {
  ...
  for (const { t, d } of rows) {
    ...
    if (isRealized(d)) {
      ...
      if (hasUsdResult(d)) {
        ...
        bucket.pctPairs.push([d.pct, d.borrowUsd]);
      }
    } else {
      bucket.open += 1;
    }
  ...
  const byCurrency = [...byCurrencyMap.values()]
    .map((b) => ({
      ...
      closed: b.closed,
      open: b.open,
      borrowed: b.valued > 0 ? b.borrowed : null,
      borrowedNative: b.closed > 0 ? b.borrowedNative : null,
  ...
  return {
    ...
    winRate: valued.length > 0 ? (wins / valued.length) * 100 : null,
    wins,
    losses: valued.length - wins,
