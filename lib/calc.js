...
export function summaryReport(trades, asOf = todayISO()) {
  ...
  const closed = rows.filter(({ d }) => d.status === STATUS.CLOSED && d.netGain !== null);
  ...
  for (const { t, d } of rows) {
    ...
    if (d.status === STATUS.CLOSED && d.netGain !== null) {
      bucket.closed += 1;
  ...
  const byCurrency = [...byCurrencyMap.values()]
    ...
    .sort((a, b) => (b.netGain ?? -Infinity) - (a.netGain ?? -Infinity));
