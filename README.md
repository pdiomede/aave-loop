...
Local web app, SQLite storage, no account and no wallet connection. USD only.
...
Opens on http://localhost:3000. The script checks Node, installs dependencies if needed, and moves to the next free port when 3000 is taken (`--kill` reclaims it instead, `--port N` picks another).
...
| Gross gain | `sell_amount - buy_amount` |
